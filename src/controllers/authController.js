...
const generateToken = (id, role) => {
  logger.info(`Generating token for user ID: ${id} with role: ${role}`);
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (user && (await bcrypt.compare(password, user.password))) {
      ...
      const blacklistEntry = await Blacklist.findOne({ token: req.token });
     if (blacklistEntry) {
       await Blacklist.deleteOne({ token: req.token });
       logger.info(`Removed token from blacklist on login: ${email}`);
     }
      await user.save();
...
export const logoutUser = async (req, res) => {
  ...
  const token = req.token; // Assuming the token is set in req.token by auth middleware
  try {
